...
date: 2024-10-16
tags: []
cover: "17027238666311_IsJ26UWXB6WbcXSBUjXniQ.png"
categories: ["tech_blog"]
author: "quirkybird"
summary_text: "跨域指协议、域名、端口任一不同。`<img>`、`<link>`、`<script>`标签可跨域。解决跨域的常见方法有：利用`<script>`标签的JSONP（仅GET）；后端设置`Access-Control-Allow-Origin`的CORS；用于窗口通信的postMessage()；不受同源策略限制的WebSocket；以及服务器代理转发（如Nginx反向代理）。"
---
