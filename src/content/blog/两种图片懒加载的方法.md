...
author: "quirkybird"
summary_text: "文章介绍了两种图片懒加载方法。第一种是利用`Element.getBoundingClientRect`，通过判断图片元素相对于视口的位置来动态加载图片，并监听滚动事件。第二种是利用`Intersection Observer API`，通过创建观察器来监听图片元素是否进入视口，一旦进入则加载图片。两种方法都将图片的真实地址预存在`data-src`中，并在图片进入视口时将其赋值给`src`属性。"
---
